...
//        therapist.getPrograms().add(program);
    ...
    public String getNextProgramId() {
        ...
        if (lastId.isPresent() && lastId.get().matches("TP\\d{3}")) {
            String lastID = lastId.get();
            int numericPart = Integer.parseInt(lastID.substring(2)); // changed from 1 to 2
            numericPart++;
    ...
    @Override
    public TherapyProgramDTO getProgramById(String id) {
        Optional<TherapyProgram> byId = therapyProgramDAO.findById(id);
        if (byId.isPresent()) {
            TherapyProgram program = byId.get();
